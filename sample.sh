...
chmod +x {file.sh} # +x gives all users rights to the file
...
ls # list directory, -R makes this recursive
grep # searches for strings in whatever
ls | grep word # searches for word in the output of ls # steve recommended a better way of doing this:
find *string* | grep other_string # this is recursive and automatically prints the full path 
# find is somehow blind to cetrain folders in root
ls -d -1 $PWD/**/*/*/* | grep string # this will find at the sub folder branch level of /*
...
scp source_file user@host:directory/target_file # writes from local computer to host computer
scp user@host:directory/source_file target_file # writes from host to local
scp -r user@host:directory/source_folder target_folder # writes from host to local recursively
...
sudo mount myiso.iso /mnt/iso/ -t iso9660 -o ro,loop=/dev/loop0 # mount .iso
...
if [ -z "$1" ] # -z returns true if the arguemnt is of zero length
then
...
I wrote to unpack 'img.tiff' from files:
for f in *0A*; do cp ${f}/img.tiff img-${f}-img.tiff;  done
